...
function leverage(static,dynamic)
    local dynamic_right = dynamic.left + dynamic.width
    local dynamic_down = dynamic.top + dynamic.heigth
    local static_right = static.left + static.width
    local static_down = static.top + static.heigth

    local s_x = (static.left +  static_right) / 2
    local s_y = (static.top +   static_down) / 2
    local d_x = (dynamic.left + dynamic_right) / 2
    local d_y = (dynamic.top +  dynamic_down) / 2
    
    local distance = (s_x - d_x) ^ 2
    distance=distance+(s_y - d_y) ^ 2
    distance=math.sqrt(distance)
    
    local max_distance = ((static.width + dynamic.width) / 2) ^ 2
    max_distance= max_distance + ((static.heigth + dynamic.heigth) / 2) ^ 2
    max_distance = math.sqrt(max_distance)
    
    local leverage = distance / max_distance
