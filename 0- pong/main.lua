...
function love.keypressed(key)
    if game_state == 'select' then -- select state
        ...
        if key == "t" then -- PvC
            ...
            enemy = Player(VIRTUAL_WIDTH / 2, 0, nil, player_heigth, 0, nil, nil, ball.speed * 2 / 3, false, true,
                false, false) -- enemy
            player1 = Player(VIRTUAL_WIDTH / 2, VIRTUAL_HEIGTH - line_thick - player_heigth, nil, player_heigth, 1,
                'left', 'right', ball.speed, false, false, false, true) -- player 1
            player2 = nil
...
function love.draw()
    ...
    -- FPS calculation
    if DEBUG then
        love.graphics.print("FPS: " .. love.timer.getFPS(), VIRTUAL_WIDTH - 50, 0)
    end
    ...
    if game_state == "select" then
        ...
        local list_of_text = {
            [0] = {
                text = "Click \"Esc\" to Exit"
            },
            [1] = {
                text = "Click \"t\" to play vs computer"
            },
            [2] = {
                text = "Click \"y\" to play vs human"
            },
            [3] = {
                text = "Click \"u\" to play with human vs computer"
            },
            [4] = {
                text = "Click \"i\" to play vs human vs computer"
            }
        }
    ...
    elseif game_state == 'PvC' or game_state == 'PvP' or game_state == 'PPvC' or game_state == 'PvPvC' then -- play state
        ...
        if game_state == 'PvC' then
            ...
            if not (player1 == nil) then
                love.graphics.print(player1.score, left, top + VIRTUAL_HEIGTH / 4)
            end
            if not (enemy == nil) then
                love.graphics.print(enemy.score, left, top - VIRTUAL_HEIGTH / 4)
            end
    ...
    elseif game_state == 'victory' then
        print_line({
            ...
            [1] = {
                text = "Press \"space\" to go to main menu",
                font = font.menu
            }
        }, 2)
    elseif game_state == 'lost' then
    ...
    elseif game_state == 'pause' then
        local list_of_text = {
            ...
            [2] = {
                text = "Click \"space\" to continue playing"
            }
        }
